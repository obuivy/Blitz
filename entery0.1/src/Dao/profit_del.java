...
@WebServlet(name = "profit_del")
public class profit_del extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String sql = "DELETE FROM profit WHERE num = '" + num + "';";
        update_sql(sql);

        response.getWriter().write("1秒后跳到主页");
        //设置1秒钟跳转
        response.setHeader("refresh", "0;url=/entery0_1_war_exploded/display_profit.jsp?year=" + str_year + "&month=" + str_month);
